...
if (!isset($_SERVER['HTTP_REFERER'])) {
    ...
    echo "<h1 style='color: red'>" . _("레퍼러가 존재하지 않습니다.") . "</h1>";
}
if (strpos($_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST']) === false) {
    header($_SERVER['SERVER_PROTOCOL'] . ' 500 Internal Server Error', true, 500);
    echo "<h1 style='color: red'>" . _("올바르지 않은 레퍼러입니다.") . "</h1>";
}
...
if ($_GET) {
    if (isset($_GET['mode']) && isset($_GET['comment'])) {
        if ($_GET['mode'] === 'exchange') {
            ...
            if ($row === false) {
                ...
                if ($row['author'] != $_SESSION['ID']) {
                    header($_SERVER['SERVER_PROTOCOL'] . ' 500 Internal Server Error', true, 500);
                    echo "<h1 style='color: red'>" . _("댓글을 쓴 사용자만이 지울 수 있습니다.") . "</h1>";
                    exit;
                }
                else {
        ...
        header($_SERVER['SERVER_PROTOCOL'] . ' 500 Internal Server Error', true, 500);
        echo "<h1 style='color: red'>" . _("파라미터가 올바르지 않습니다!") . "</h1>";
    }
}
else {
    header($_SERVER['SERVER_PROTOCOL'] . ' 500 Internal Server Error', true, 500);
